    ...
    def line_search(self, oracle, x_k, d_k, previous_alpha=None):
        ...
        if self._method == 'Wolfe':
            ...
            for _ in range(60):
                ...
                armijo_ok = phi_alpha <= phi0 + self.c1 * alpha * dphi0
                wolfe_ok = abs(dphi_alpha) <= self.c2 * abs(dphi0)
        ...
        if self._method == 'Armijo':
            alpha = previous_alpha if previous_alpha is not None else self.alpha_0
            for _ in range(100):
                if phi(alpha) <= phi0 + self.c1 * alpha * dphi0:
                    return alpha
                alpha *= 0.5
...
def gradient_descent(oracle, x_0, tolerance=1e-5, max_iter=10000,
    ...
    def record_state(f_val, grad, x):
        if not trace:
            return
        history['func'].append(np.squeeze(f_val))
    ...
    for _ in range(max_iter):
        ...
        f_k = oracle.func(x_k)
        g_k = oracle.grad(x_k)
        if not (np.isfinite(f_k) and np.all(np.isfinite(g_k))):
            return x_k, 'computational_error', history

        record_state(f_k, g_k, x_k)
        if display:
            print(f"GD step: f={f_k}, ||g||={np.linalg.norm(g_k)}, alpha={alpha}")
...
def newton(oracle, x_0, tolerance=1e-5, max_iter=100,
    ...
    for _ in range(max_iter):
        ...
        record_state(f_k, g_k, x_k)
        if display:
            print(f"Newton step: f={f_k}, ||g||={np.linalg.norm(g_k)}, alpha={alpha}")
